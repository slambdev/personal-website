...
const ExperienceHeader = styled(FlexBox)(
  sx([
    {
...
export const Experience = ({ experience, sx: sxProps }: ExperienceProps) => {
  const sortedTechnologies: string[] = _.orderBy(
      ...
      <Box sx={{ width: `100%` }}>
        <ExperienceHeader>
          <FlexBox sx={{ alignItems: `center` }}>
            <Typography
              variant='h5'
              sx={{ paddingLeft: gap, paddingRight: fourthGap }}
            >
              {experience.team
          ...
          <Typography variant='h5' sx={{ paddingX: gap }}>{`${formatDate(
            experience.startDate,
          )} - ${formatDate(experience.endDate)}`}</Typography>
        </ExperienceHeader>
        <ExperienceBox>
          <Typography variant='h3' sx={{ paddingBottom: halfGap }}>
            {experience.role}
          </Typography>
          ...
          <FlexBox sx={{ flexWrap: `wrap` }}>
            {sortedTechnologies.map((tech, i) => (
              <Chip
                key={i}
                label={tech}
                sx={{ marginRight: fourthGap, marginBottom: fourthGap }}
              />
            ))}
          </FlexBox>
        </ExperienceBox>
